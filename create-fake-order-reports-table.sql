...
CREATE TABLE IF NOT EXISTS fake_order_reports (
  ...
  shop_order_count BIGINT DEFAULT 0, -- C 列：单量 (店铺)
  product_order_count BIGINT DEFAULT 0, -- D 列：单量 (单品)
  
  -- 订单信息
  order_id TEXT,                 -- E 列：id
  sub_order_no TEXT,             -- F 列：子订单编号
  main_order_no TEXT,            -- G 列：主订单编号
  ...
  paid_amount NUMERIC(12,2) DEFAULT 0, -- S 列：买家实付金额
  ...
  -- 时间信息
  order_created_at TIMESTAMPTZ,  -- V 列：订单创建时间
  order_paid_at TIMESTAMPTZ,     -- W 列：订单付款时间
  ship_time TIMESTAMPTZ,         -- AD 列：发货时间
  should_ship_at TIMESTAMPTZ,    -- AJ 列：应发货时间
  
  -- 渠道信息
  taoxianda_channel TEXT,        -- X 列：淘鲜达渠道
 ...
 百亿补贴_info TEXT,            -- AN 列：百亿补贴超链订单信息
  chain_half_managed_info TEXT,  -- AO 列：超链半托管订单信息
  ...
  buyer_message TEXT,            -- AC 列：主订单买家留言
  ...
  compensation_paid_at TIMESTAMPTZ, -- AI 列：主动赔付出账时间

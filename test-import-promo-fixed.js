...
// Excel 表头映射（按列顺序）
const HEADER_MAP = [
  '日期', '主体 ID', '主体类型', '主体名称', '展现量', '点击量', '花费', '点击率', '平均点击花费', '千次展现花费',
  '总预售成交金额', '总预售成交笔数', '直接预售成交金额', '直接预售成交笔数', '间接预售成交金额', '间接预售成交笔数',
  '直接成交金额', '间接成交金额', '总成交金额', '总成交笔数', '直接成交笔数', '间接成交笔数',
  '点击转化率', '投入产出比', '含预售投产比', '总成交成本', '总购物车数', '直接购物车数', '间接购物车数', '加购率',
  '收藏宝贝数', '收藏店铺数', '店铺收藏成本', '总收藏加购数', '总收藏加购成本', '宝贝收藏加购数', '宝贝收藏加购成本',
  '总收藏数', '宝贝收藏成本', '宝贝收藏率', '加购成本', '拍下订单笔数', '拍下订单金额', '直接收藏宝贝数', '间接收藏宝贝数',
  '优惠券领取量', '购物金充值笔数', '购物金充值金额', '旺旺咨询量', '引导访问量', '引导访问人数', '引导访问潜客数', '引导访问潜客占比',
  '入会率', '入会量', '引导访问率', '深度访问量', '平均访问页面数', '成交新客数', '成交新客占比', '会员首购人数',
  '会员成交金额', '会员成交笔数', '成交人数', '人均成交笔数', '人均成交金额', '自然流量转化金额', '自然流量曝光量',
  '平台助推总成交', '平台助推直接成交', '平台助推点击', '宝贝优惠券抵扣金额', '宝贝优惠券撬动总成交', '宝贝优惠券撬动直接成交', '宝贝优惠券撬动点击'
]

// Excel 日期转换
function excelDateToJSDate(excelDate) {
  if (!excelDate || typeof excelDate !== 'number') return new Date().toISOString().split('T')[0]
...
async function importProductPromo() {
  ...
  const targetFile = files.find(f => f.includes('商品推广') || (f.includes('20260330') && f.includes('xls')))
  if (!targetFile) {
    console.error('❌ 未找到商品推广文件')
    return
  }
  ...
  console.log('使用预设表头:', HEADER_MAP.length, '列\n')
  
  // 转换为对象数组（使用预设表头）
  const data = rawData.slice(1).map(row => {
    const obj = {}
    HEADER_MAP.forEach((header, i) => {
      obj[header] = row[i] !== undefined ? row[i] : ''
    })
    return obj
  }).filter(obj => obj['日期'] || obj['主体 ID'])
  ...
  const rowsToInsert = data.map(row => ({
    shop_id: SHOP_ID,
    report_date: excelDateToJSDate(row['日期']),
  ...
  for (let i = 0; i < uniqueRows.length; i += batchSize) {
    ...
    console.log(`\n插入批次 ${Math.floor(i/batchSize)+1}/${Math.ceil(uniqueRows.length/batchSize)}: ${batch.length} 行...`)
    ...
    if (insertError) {
      console.error(`❌ 批次 ${Math.floor(i/batchSize)+1} 失败:`, insertError.message)
    } else {
      totalInserted += insertData?.length || batch.length
      console.log(`✅ 批次 ${Math.floor(i/batchSize)+1} 成功：${insertData?.length || batch.length} 行`)
    }
  }
  
  console.log(`\n📊 总计插入：${totalInserted}/${uniqueRows.length} 行`)
